...
describe("Integration-Testing HowLongToBeatService", () => {
  describe("Test for detail()", () => {
    it("should load entry for 2224 (Dark Souls)", () => {
      return new HowLongToBeatService().getDetail("2224").then(entry => {
        assert.isNotNull(entry);
        assert.strictEqual(entry.id, "2224");
        assert.strictEqual(entry.name, "Dark Souls");
        assert.strictEqual(
          entry.imageUrl,
          "https://howlongtobeat.com/gameimages/Dark_Souls_Cover_Art.jpg"
        );
    ...
    it("should fail to load entry for 123 (404)", () => {
      return assert.isRejected(new HowLongToBeatService().getDetail("123"));
    });
  });
  ...
  describe("Test for search()", () => {
    ...
    it("should have at least 3 search results when searching for dark souls III", () => {
      return new HowLongToBeatService()
        ...
        .then(result => {
          ...
          assert.strictEqual(
            result[0].imageUrl,
            "https://howlongtobeat.com/gameimages/Dark_souls_3_cover_art.jpg"
          );
    ...
    it('Entries without any time settings (e.g. "Ambers boom") should have a zero hour result', () => {
      return new HowLongToBeatService().search("Ambers boom").then(result => {
        console.log(result);
        assert.isNotNull(result);
    ...
    it("should be able to use pagination", () => {
      let service = new HowLongToBeatService();
